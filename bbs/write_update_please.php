...
if( $_POST['wr_1'] < 10 ) {
    $msg[] = '아몬드볼은 최소 10 ABOLL 부터 가능합니다.';
}

if ( $wr_1 + $wr_7 > $member['mb_point'] ) {
    $msg[] = '보유 아몬드보다 아몬드를 많이 사용합니다. 충전해주세요.';
};

if($board['bo_use_category']) {
//    $ca_name = $wr_3; -- 카테고리 수정하기 ksm
    ...
    if(!$ca_name) {
        ...
        // 카테고리 수정하기 ksm start - 기존대로 하기로 함
//        $ca_name = clean_xss_tags($ca_name);
//        $ca_name = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\^\*]/", "", $ca_name);
//        $category_list = explode("|", $board['bo_category_list']);
//        foreach( $category_list as $item ):
//            if( strpos($item, $wr_3) !== false) {
//                $ca_name = $item;
//            }
//        endforeach;
        // 카테고리 수정하기 ksm end
...
// 글작성 기간과 우선노출하기 기간 확인 후 우선노출이 기간보다 길 경우에는 저장안되게 wr_6 = 우선노출 , wr_2 = 게시기간
if (isset($_POST['wr_6'])) {
    if ( $_POST['wr_6'] > $_POST['wr_2'] ) {
        $msg[] = '<strong>우선노출하기는 기간보다 같거나 빠른일자로 선택 해주세요.</strong>';
    }
}
...
// 글쓰기 완료시 포인트 차감
$res = sql_fetch(" select mb_id, wr_1 from $write_table where wr_parent = '$wr_id' "); //채택 포인트

if ( $res['wr_1'] > 0 ) { // 차감 포인트가 0 이상일 경우에만.
...
//포인트 테이블 추가
    $sql = " insert into $g5[point_table]
			...
			po_content = '$wr_id-번글 해주세요 게시판 작성 차감',
			po_point = '$wr_1',
			po_mb_point = '$mb_point',
			po_expire_date = '$po_expire_date',
			po_rel_table = '$bo_table',
			po_rel_id = '$wr_id',
			po_rel_action = '해주세요 글작성'
		";
    sql_query($sql);

//멤버 테이블 포인트 업데이트
    sql_query(" update $g5[member_table] set mb_point = '$mb_point' where mb_id = '$res[mb_id]' ");
}

// 우선노출 값이 있다면
if ( $wr_7 > 0 ) {
    $wosun = $wr_7 * (-1); // 차감 포인트
    $mb_point = $mb_point - $wosun;
    $po_expire_date = '9999-12-31';

    $sql = " insert into $g5[point_table]
			set mb_id = '$member[mb_id]',
			po_datetime = '".G5_TIME_YMDHIS."',
			po_content = '$wr_id-번글 해주세요 게시판 우선노출 사용',
			po_point = '$wosun',
			po_mb_point = '$mb_point',
			po_expire_date = '$po_expire_date',
			po_rel_table = '$bo_table',
