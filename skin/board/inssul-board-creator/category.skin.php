...
<div class="custom_category <?php echo (G5_IS_MOBILE) ? ' list-category-mobile' : '';?>">
	<div class="category_slide_wrap tabs div-tab<?php echo $category_tabs;?> hidden-xs">
		<ul class="category_slide_lists <?php echo ($boset['ctype'] == "1") ? ' nav-justified' : '';?><?php echo ($cate_w) ? ' text-center' :'';?>">
			<li class="category_slide_item <?php echo (!$sca) ? 'active' : ''; ?><?php echo $cate_w;?>">
				<a href="./board.php?bo_table=<?php echo $bo_table;?>">
					<span class="item_img"></span>
					<span class="item_txt">전체보기</span>
				<?php /*if(!$sca) echo '('.number_format($total_count).')';*/?></a>
			</li>
			<?php for ($i=0; $i < $ca_cnt; $i++) { ?>
				<li class="category_slide_item <?php echo ($categories[$i] === $sca) ? 'active' : '';?><?php echo $cate_w;?>">
					<a href="./board.php?bo_table=<?php echo $bo_table;?>&amp;sca=<?php echo urlencode($categories[$i]);?>">
					<span class="item_img"></span>
					<span class="item_txt"><?php echo $categories[$i];?><?php if($categories[$i] === $sca) /*echo '('.number_format($total_count).')'*/;?></span>
					</a>
				</li>
			<?php } ?>
		</ul>
