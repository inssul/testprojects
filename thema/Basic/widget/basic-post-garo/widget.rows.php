			...
			<div class="post-item">
				<a href="<?php echo $img[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?>>
					<div class="img-wrap<?php echo $is_hover;?>" style="padding-bottom:<?php echo $img_h;?>%;">
						...
						<!-- 여분필드 (아몬드볼) 시작 -->
						<div class="aball_quantity">
							여분필드 출력 해야할 위치..asasasas
							
							<?php print_r($img); echo $img[$i]['content'] ?>
							aslkdjasl
						</div>
						<!-- 여분필드 (아몬드볼) 끝 -->
				...
				<div class="post-content<?php echo $is_center;?>">
					<div class="post-subject" style="height:<?php echo $text_height;?>px;">
						<a href="<?php echo $img[$i]['href'];?>"<?php echo $is_modal_js;?><?php echo $target;?>>
							<?php echo $wr_icon;?>
							<?php echo $img[$i]['subject'];?> <!-- 제목 -->
							<?php if($is_cont) { ?>
								<div class="post-text">
									<?php echo apms_cut_text($img[$i]['content'], 100);?> <!-- 글 내용 -->
								</div>
							<?php } ?>
						</a>
					</div>
					<div class="post-text post-ko txt-short ellipsis<?php echo $is_center;?><?php echo $is_details;?>">
						<?php echo $img[$i]['name'];?> <!-- 글쓴이 -->
						...
						<?php if($is_idate) { ?>
							...
							<span class="txt-normal">
								<?php echo ($is_dtxt) ? apms_datetime($img[$i]['date'], $is_dtype) : date($is_dtype, $img[$i]['date']); ?><-  date
							</span>
						<?php } ?>
